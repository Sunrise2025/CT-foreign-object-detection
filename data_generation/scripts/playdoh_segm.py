...
def segment(input_folder, output_folder, otsu_classes):  # 对重建的体积数据进行分割
    ...
    print("====== Multi-Otsu 分割日志 ======")
    print(f"Otsu 阈值（由低到高）: {thr}")
    # 1. 先按灰度区间生成 mask（不赋具体材料语义）
    mask_0 = vol <= thr[0]
    mask_1 = (vol > thr[0]) & (vol <= thr[1])
    mask_2 = vol > thr[1]
    # 2. 计算每一类的平均灰度（用于判断材料）
    mean_0 = vol[mask_0].mean() if np.any(mask_0) else 0
    mean_1 = vol[mask_1].mean() if np.any(mask_1) else 0
    mean_2 = vol[mask_2].mean() if np.any(mask_2) else 0
    count_0 = np.sum(mask_0)
    count_1 = np.sum(mask_1)
    count_2 = np.sum(mask_2)

    print(f"类别A：体素数 = {count_0}, 平均灰度 = {mean_0:.2f}")
    print(f"类别B：体素数 = {count_1}, 平均灰度 = {mean_1:.2f}")
    print(f"类别C：体素数 = {count_2}, 平均灰度 = {mean_2:.2f}")
    # 3. 按“平均灰度从小到大”排序 → 空气 / 泥土 / 石头
    means = np.array([mean_0, mean_1, mean_2])
    counts = np.array([count_0, count_1, count_2])
    masks = [mask_0, mask_1, mask_2]

    order = np.argsort(means)  # 灰度从低到高
    ...
    print("\n===== 材料判定结果 =====")
    print(f"空气   → 类别 {air_idx}（平均灰度 {means[air_idx]:.2f}）")
    print(f"泥土   → 类别 {playdoh_idx}（平均灰度 {means[playdoh_idx]:.2f}）")
    print(f"石头   → 类别 {stone_idx}（平均灰度 {means[stone_idx]:.2f}）")

    # 4. 显式写入最终分割标签
    segm_vol[:] = 0  # 0 = 空气
    segm_vol[masks[playdoh_idx]] = 1  # 1 = Playdoh
    segm_vol[masks[stone_idx]] = 2  # 2 = Stone

    print("\n===== 最终体素统计 =====")
    print(f"空气体素数   : {np.sum(segm_vol == 0)}")
    print(f"泥土体素数   : {np.sum(segm_vol == 1)}")
    print(f"石头体素数   : {np.sum(segm_vol == 2)}")
    print("====== 分割完成 ======\n")
