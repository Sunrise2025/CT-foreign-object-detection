...
def postprocess_proj(stone_2_path, stone_1_path, output_folder):
    print(f"【输入路径】stone_2_path = {stone_2_path}")
    print(f"【输入路径】stone_1_path = {stone_1_path}")
    print(f"【输出路径】output_folder = {output_folder}")
    config_fname = "playdoh.ini"  # 设置配置文件名

    stone_2_path = Path(stone_2_path)
    os.makedirs(stone_2_path, exist_ok=True)
    stone_1_path = Path(stone_1_path)
    os.makedirs(stone_1_path, exist_ok=True)
    playdoh_basic_augmentation(config_fname, stone_2_path, stone_1_path)
    ...
    # 6. 复制扫描设置文件到输出文件夹
    shutil.copy(os.path.join(stone_2_path, 'scan settings.txt'), output_folder)

obj_id = 109
base_root = r"M:\m\pycharm_project\project1\Data\Real\Playdoh0-3"
stone_2_path = rf"{base_root}\Playdoh4\Object{obj_id}_Scan20W"
stone_1_path = rf"{base_root}\基多类生1类-留1\Object{obj_id}_Scan20W"
output_folder = rf"{base_root}\基多类生1类-留1后置中心\Object{obj_id}_Scan20W"
...
# 对每个输入文件夹进行后处理
postprocess_proj(stone_2_path,stone_1_path, output_folder)
# 体积分割切片处理，生成虚拟异物样本
