...
class Projector(object):
    ...
    def poly_fp(self, voltage):
        ...
        main_attenuation = np.zeros(self.energy_bins)  # 对主物质的衰减系数初始化（可以是空的，稍后根据材质赋值）
        foreign_attenuation = np.zeros(self.energy_bins)  # 对外来物质的衰减系数初始化（同样可以是空的）
        spectrum_fractions = np.zeros(self.energy_bins)
        main_projection = self.project_material(1)  # 假设主物质的编号为1
        foreign_projection = self.project_material(2)  # 假设外来物质的编号为2

        for i in range(self.energy_bins):
            temp_spectral_projection = main_attenuation[i] * main_projection + foreign_attenuation[
                i] * foreign_projection
    ...
    def create_projection(self, start_num, folder, voltage):  #对投影 & 亮场 添加噪声
