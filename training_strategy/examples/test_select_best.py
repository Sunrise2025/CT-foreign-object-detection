...
def test(config, data_folder, epoch_file, data_type, nn_type):
    ...
    # logger2.info("---------------------新策略，Testing started.---------------------")
    # 生成训练集目标标签
    ...
    logger2.info("/////////新策略Test，模型名：{}, 最佳迭代：{} , 准确率：{:.6f}".format(folder.name, best_epoch, accuracy))
    total_time = time.time() - start_time
    # 将总时间转换为小时
    total_hours = total_time / 60
    # logger2.info(f"新策略Testing completed in {total_hours:.2f} 分钟")
    # logger2.info("---------------------新策略，Testing end.---------------------")
...
if __name__ == "__main__":
    ...
    test_file = config['General']['test_file1']
    ...
    for folder in tqdm(subfolders):
        # 查找以 "select_best_" 开头的文件
        saves = [x.stem for x in folder.glob("select_best_*.torch")]

        # 如果找到了select_best_*文件，直接选取第一个
        if saves:
            best_epoch = int(saves[0].split('_')[-1])  # 提取文件名中的 epoch 部分
            epoch_file = folder / "select_best_{}.torch".format(best_epoch)

        else:
            raise FileNotFoundError(f"No select_best_*.torch file found in {folder}")

        accuracy = test(config, folder, epoch_file, data_type, nn_type)
        print("新策略test，模型名：{}, 新策略最佳迭代: {}".format(folder.name, best_epoch))
