...
def test(config, data_folder, epoch_file, data_type, nn_type):
    ...
    # logger2.info("---------------------原策略，Testing started.---------------------")
    # 生成训练集目标标签
    ...
    logger2.info("/////////原策略Test，模型名：{}, 最佳迭代：{} , 准确率：{:.6f}".format(folder.name, best_epoch, accuracy))
    total_time = time.time() - start_time
    # 将总时间转换为小时
    total_hours = total_time / 60
    # logger2.info(f"原策略Testing completed in {total_hours:.2f} 分钟")
    # logger2.info("---------------------原策略，Testing end.---------------------")
...
if __name__ == "__main__":
    ...
    test_file = config['General']['test_file2']
    ...
    for folder in tqdm(subfolders):
        saves = [x.stem for x in folder.glob("*.torch") if x.stem.isdigit()]

        if saves:
            best_epoch = int(saves[0])  # 这里不用 split 了
            epoch_file = folder / "{}.torch".format(best_epoch)
        else:
            raise FileNotFoundError(f"No *.torch file found in {folder}")


        accuracy = test(config, folder, epoch_file, data_type, nn_type)
        print("原策略test，模型名：{}, 原默认最佳迭代: {}".format(folder.name, best_epoch))
